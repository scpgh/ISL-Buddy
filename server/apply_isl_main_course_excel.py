...
excel_path = "e:/ISL_project/server/isl_main_course.xlsx"
v8_path = "e:/ISL_project/server/isl-course-data-v8.json"

# 1. Parse Excel rows directly
excel_rows = []
    ...
    for row in s_root.findall('.//a:row', s_ns):
        ...
        if len(cells) >= 2:
            ...
            if v1_tag is not None and v1_tag.text:
                val1 = shared_strings[int(v1_tag.text)] if c1.attrib.get('t') == 's' else v1_tag.text
            
            v2_tag = c2.find('a:v', s_ns)
            if v2_tag is not None and v2_tag.text:
                val2 = shared_strings[int(v2_tag.text)] if c2.attrib.get('t') == 's' else v2_tag.text

            if val1 and val2 and val2.startswith('http'):
                excel_rows.append((str(val1).strip(), str(val2).strip()))

print("Found Excel rows count:", len(excel_rows))
for r in excel_rows:
    print(r)

# Map key -> url
# Row 0: ('1', 'https://youtu.be/5PF6JXzYyUI') -> Guideline / Course Intro
# Row 1: ('1.1000000000000001', 'https://youtu.be/n42ohSmbAFI') -> Lesson 1.1
# Row 2: ('1.2', 'https://youtu.be/5vHmvYA8Z6Q') -> Lesson 1.2
# ...

url_map = {}
for code, url in excel_rows:
    if code == '1' or code == '1.0':
        key = '0'
    else:
        try:
            f_val = float(code)
...
print("URL MAP KEYS:", list(url_map.keys()))
...
# Guideline Video 0
guideline_url = url_map.get('0', 'https://youtu.be/5PF6JXzYyUI')
g_id = guideline_url.split('/')[-1]
...
guideline_lesson = {
  ...
  "videoUrlEnglish": f"https://www.youtube.com/embed/{g_id}",
...
for m in course_v8["modules"]:
    if "lessons" in m and isinstance(m["lessons"], list):
        for l in m["lessons"]:
            ...
            raw_url = url_map.get(topicCode, l.get("video_url"))
            vid_id = raw_url.split('/')[-1].replace('watch?v=', '').split('&')[0]
...
for target in [r"e:\ISL_project\client\src\data\islData.js", r"e:\ISL_project\src\data\islData.js"]:
    if os.path.exists(target):
        ...
        print(f"Successfully applied exact YouTube video IDs from isl_main_course.xlsx to: {target}")
